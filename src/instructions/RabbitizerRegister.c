...
const char *RabbitizerRegister_getNameR4000AllegrexVScalar(uint8_t regValue) {
    assert(regValue < ARRAY_COUNT(RabbitizerRegister_R4000AllegrexVScalar_Names));

    return RabbitizerRegister_R4000AllegrexVScalar_Names[regValue][RabbitizerConfig_Cfg.regNames.namedRegisters ? 1 : 0];
}
...
const RabbitizerRegisterDescriptor *RabbitizerRegister_getDescriptor_R4000AllegrexVScalar(uint8_t regValue) {
    ...
    return &RabbitizerRegister_R4000AllegrexVScalar_Descriptors[regValue];
}
